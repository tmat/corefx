...
namespace System.Reflection.Metadata.Ecma335
{
    ...
    internal unsafe struct BlobStreamReader
    {
        ...
        internal byte[] GetBytes(BlobHandle handle)
        {
            ...
            int offset = handle.GetHeapIndex();
            int bytesRead;
        ...
        internal BlobReader GetBlobReader(BlobHandle handle)
        {
            ...
            int offset, size;
            Block.PeekHeapValueOffsetAndSize(handle.GetHeapIndex(), out offset, out size);
        ...
        internal BlobHandle GetNextHandle(BlobHandle handle)
        {
            ...
            int offset, size;
            if (!Block.PeekHeapValueOffsetAndSize(handle.GetHeapIndex(), out offset, out size))
            {
                return default(BlobHandle);
            }
            ...
            return BlobHandle.FromIndex(nextIndex);
        }
    ...
    internal struct UserStringStreamReader
    {
        ...
        {
            int offset, size;
            if (!Block.PeekHeapValueOffsetAndSize(handle.Offset, out offset, out size))
            {
                return string.Empty;
            }
